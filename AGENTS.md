...
- 在线网络目录长按文件夹可递归缓存其中所有受支持媒体。任务先扫描、再最多并行处理 3 个文件；进度保存在 `NetworkBrowserViewModel`，模态弹窗阻止其他页面操作，并只通过明确的“取消缓存”按钮中止。点击取消后界面必须立即切换为可关闭的“已取消”状态，不能等待阻塞式 SMB 读取返回；下载层会关闭当前 SMB 文件句柄并清理未完成的 `.tmp`，但必须保留此前以及本次已经完成的缓存。
